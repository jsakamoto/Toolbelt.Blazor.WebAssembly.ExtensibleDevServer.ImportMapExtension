...
fingerprinted URL with whatever is on disk right now. So the moment you edit an imported `.js` file
and reload, the browser fetches the new content under the old name. The digest no longer matches, and
...
That is all. Run the app, edit an imported `.js` file, reload, and it works. Publish, and
`{importmap}` is replaced with the real digest.
